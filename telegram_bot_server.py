...
class BotState:
    ...
    def initialization(self, server_addr='localhost', server_port=12345,
                       stopping_addr='localhost', stopping_port=12346):
        server_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        server_socket.bind((server_addr, server_port))
        server_socket.setblocking(False)
        stopping_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        stopping_socket.bind((stopping_addr, stopping_port))
